...
/// <summary>
/// Repository for managing outbox messages in Order service.
/// Provides transactional outbox pattern for reliable event publishing.
/// </summary>
public sealed class OrderOutboxRepository : IOutboxRepository
{
    private readonly OrderInfrastructureDbContext _dbContext;

    public OrderOutboxRepository(OrderInfrastructureDbContext dbContext)
    {
    ...
    public async Task<List<OutboxMessage>> GetFailedMessagesAsync(
        int maxRetries = 3,
        int batchSize = 100,
        CancellationToken cancellationToken = default)
    {
