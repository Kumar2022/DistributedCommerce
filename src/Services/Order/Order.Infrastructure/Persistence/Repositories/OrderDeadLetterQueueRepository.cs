...
/// <summary>
/// Repository for managing dead letter queue messages in Order service.
/// Stores permanently failed messages for manual intervention.
/// </summary>
public sealed class OrderDeadLetterQueueRepository : IDeadLetterQueueRepository
{
    private readonly OrderInfrastructureDbContext _dbContext;

    public OrderDeadLetterQueueRepository(OrderInfrastructureDbContext dbContext)
    {
    ...
    public async Task MarkAsReprocessedAsync(
        Guid id, 
        string? notes = null, 
        CancellationToken cancellationToken = default)
    {
        ...
        if (message != null)
        {
            ...
            {
                message.OperatorNotes = string.IsNullOrEmpty(message.OperatorNotes) 
                    ? notes 
    ...
    public async Task<DlqStatistics> GetStatisticsAsync(
        DateTime? fromDate = null, 
        CancellationToken cancellationToken = default)
    {
        var query = _dbContext.DeadLetterMessages.AsQueryable();
        ...
        var messagesByService = await query
            .GroupBy(m => m.ServiceName)
            .Select(g => new { Service = g.Key, Count = g.Count() })
            .ToDictionaryAsync(x => x.Service, x => x.Count, cancellationToken);

        var messagesByEventType = await query
    ...
    public async Task CleanupReprocessedAsync(
        TimeSpan olderThan, 
        CancellationToken cancellationToken = default)
    {
        var cutoffDate = DateTime.UtcNow - olderThan;
        
        var oldMessages = await _dbContext.DeadLetterMessages
            .Where(m => m.Reprocessed && m.MovedToDlqAt < cutoffDate)
            .ToListAsync(cancellationToken);
